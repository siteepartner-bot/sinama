...
export function CallControls({
  ...
  const { roomState, toggleMic, toggleCamera, toggleScreenShare, leaveRoom } = useRoom();

  if (!roomState || !roomState.currentUser) return null;

  const me = roomState.currentUser;
  ...
  return (
    <div className="flex flex-wrap items-center justify-between gap-4 p-4 bg-[#12141c] border border-zinc-800/80 rounded-2xl shadow-xl w-full" id="call-controls-container">
      ...
      <div className="flex items-center gap-2">
        ...
        <span className="text-xs font-semibold text-zinc-300">متصل به سرور دمو</span>
        <span className="text-[10px] text-zinc-500">({me.name})</span>
      </div>
      ...
      <div className="flex items-center gap-3">
        ...
        <motion.button
          ...
          title={me.isMicActive ? 'قطع میکروفون' : 'فعال‌سازی میکروفون'}
          id="btn-mic-toggle"
        >
          {me.isMicActive ? <Mic className="h-5 w-5" /> : <MicOff className="h-5 w-5" />}
        </motion.button>
        ...
        <motion.button
          ...
          title={me.isCameraActive ? 'قطع دوربین' : 'فعال‌سازی دوربین'}
          id="btn-camera-toggle"
        >
          {me.isCameraActive ? <Video className="h-5 w-5" /> : <VideoOff className="h-5 w-5" />}
        </motion.button>

        {/* Screen Share Button */}
        <ScreenShareButton isSharing={me.isScreenSharing} onClick={toggleScreenShare} />
